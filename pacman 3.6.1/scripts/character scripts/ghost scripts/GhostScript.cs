...
public class GhostScript : CharacterScript
{
    ...
    public void OnGhostOverlapEntered(Area2D area) //if 2 ghosts are ontop of each other, randomly increase speed so they move away
    {
        if (hasIntersectedBefore == false)
        {
            oldSpeed = speed;
            speed = speed + (int)GD.RandRange(3, 20);
        }
        hasIntersectedBefore = true;
    }

    public void OnGhostOverlapExited(Area2D area) //when 2 ghosts are not ontop of each other reset speed back to normal.
    {
        speed = oldSpeed;
